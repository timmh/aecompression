# from: https://github.com/AdiModi96/Breaking-Down-LoRA/blob/master/lora_modules.py
...
def fake_quantize(x: torch.Tensor, precision: int = None):
    assert x.dtype == torch.float32 or x.dtype == torch.float16
    if precision is None:
        return x
    else:
        return x.clamp(-2 ** precision, 2 ** precision -1).round()
...
class LoRALinear(nn.Module):
    ...
    def get_merged_module(self) -> nn.Linear:
        effective_weight = self.base_module.weight + ((self.alpha / self.rank) * torch.matmul(fake_quantize(self.delta_weight_A, precision=self.precision), fake_quantize(self.delta_weight_B, precision=self.precision)))
        out_features, in_features = effective_weight.size()

        effective_bias = None
        if self.base_module.bias is not None:
            if self.delta_bias is not None:
                effective_bias = self.base_module.bias + (self.beta * fake_quantize(self.delta_bias, self.precision))
            else:
                effective_bias = self.base_module.bias
        else:
            if self.delta_bias is not None:
                effective_weight = (self.beta * fake_quantize(self.delta_bias, self.precision))
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.adapter_enabled:
            ...
            if self.base_module.bias is not None:
                if self.delta_bias is not None:
                    effective_bias = self.base_module.bias + (self.beta * fake_quantize(self.delta_bias, precision=self.precision))
                else:
                    effective_bias = self.base_module.bias
            else:
                if self.delta_bias is not None:
                    effective_weight = (self.beta * fake_quantize(self.delta_bias, precision=self.precision))
        else:
...
class LoRAConv1d(nn.Module):
    ...
    def get_merged_module(self) -> nn.Conv1d:
        ...
        effective_weight = self.base_module.weight + (
                (self.alpha / self.rank) * fake_quantize(torch.matmul(fake_quantize(self.delta_weight_A, precision=self.precision), fake_quantize(_B, precision=self.precision)).permute(1, 2, 0), self.precision))
        ...
        if self.base_module.bias is not None:
            ...
            if self.delta_bias is not None:
                effective_bias = (self.beta * fake_quantize(self.delta_bias, self.precision))
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.adapter_enabled:
            effective_weight = self.base_module.weight + (
                    (self.alpha / self.rank) * torch.matmul(fake_quantize(self.delta_weight_A, precision=self.precision), fake_quantize(self.delta_weight_B, precision=self.precision)).permute(1, 2, 0))
...
class LoRAConv2d(nn.Module):
    ...
    def get_merged_module(self) -> nn.Conv2d:
        ...
        if self.rank_for == 'kernel':
            ...
            effective_weight = self.base_module.weight + (
                    (self.alpha / self.rank) * torch.matmul(fake_quantize(self.delta_weight_A, precision=self.precision), fake_quantize(self.delta_weight_B, precision=self.precision)).permute(2, 3, 0, 1))
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.adapter_enabled:
            if self.rank_for == 'kernel':
                effective_weight = self.base_module.weight + ((self.alpha / self.rank) * torch.matmul(fake_quantize(self.delta_weight_A, self.precision), fake_quantize(self.delta_weight_B, self.precision)))
            elif self.rank_for == 'channels':
                effective_weight = self.base_module.weight + (
                        (self.alpha / self.rank) * torch.matmul(fake_quantize(self.delta_weight_A, self.precision), fake_quantize(self.delta_weight_B, self.precision)).permute(2, 3, 0, 1))
...
class LoRAConv3d(nn.Module):
    ...
    def get_merged_module(self) -> nn.Conv3d:
        ...
        effective_weight = self.base_module.weight + (
                (self.alpha / self.rank) * torch.matmul(fake_quantize(self.delta_weight_A, precision=self.precision), fake_quantize(self.delta_weight_B, precision=self.precision)).permute(3, 4, 0, 1, 2))
...
class LoRAConvTranspose2d(nn.Module):
    ...
    # Takes a torch.nn.ConvTranspose2d, LoRA config and builds an adapter
    def __init__(self, base_module: nn.ConvTranspose2d, lora_config: dict) -> None:
        super(LoRAConvTranspose2d, self).__init__()

        assert isinstance(base_module, nn.ConvTranspose2d), 'Invalid type! The base module should be of type `torch.nn.ConvTranspose2d`.'

        self.base_module = base_module
        for parameter in self.base_module.parameters():
            parameter.requires_grad = False
        in_channels, out_channels, kH, kW = self.base_module.weight.size()
        ...
        if self.rank_for == 'kernel':
            self.delta_weight_A = nn.Parameter(
                torch.empty(
                    size=(in_channels, out_channels, kH, lora_config['rank']),
                    dtype=self.base_module.weight.dtype,
                    device=self.base_module.weight.device
                )
            )
            self.delta_weight_B = nn.Parameter(
                torch.empty(
                    size=(in_channels, out_channels, lora_config['rank'], kW),
                    dtype=self.base_module.weight.dtype,
                    device=self.base_module.weight.device
                )
            )
            ...
            self.delta_weight_A = nn.Parameter(
                torch.empty(
                    size=(kH, kW, in_channels, lora_config['rank']),
                    dtype=self.base_module.weight.dtype,
                    device=self.base_module.weight.device
                )
            )
            self.delta_weight_B = nn.Parameter(
                torch.empty(
                    size=(kH, kW, lora_config['rank'], out_channels),
                    dtype=self.base_module.weight.dtype,
                    device=self.base_module.weight.device
                )
            )
    ...
    # Creates a new instance of type torch.nn.ConvTranspose2d with merged weight of base module and LoRA adapter
    def get_merged_module(self) -> nn.ConvTranspose2d:
        in_channels, out_channels, kH, kW = self.base_module.weight.size()

        if self.rank_for == 'kernel':
            delta_weight = torch.matmul(fake_quantize(self.delta_weight_A, precision=self.precision), fake_quantize(self.delta_weight_B, precision=self.precision))
            effective_weight = self.base_module.weight + ((self.alpha / self.rank) * delta_weight)
        elif self.rank_for == 'channels':
            delta_weight = torch.matmul(fake_quantize(self.delta_weight_A, precision=self.precision), fake_quantize(self.delta_weight_B, precision=self.precision)).permute(2, 3, 0, 1)
            effective_weight = self.base_module.weight + ((self.alpha / self.rank) * delta_weight)
        ...
        merged_module = nn.ConvTranspose2d(
            in_channels=in_channels,
            out_channels=out_channels,
            kernel_size=(kH, kW),
            stride=self.base_module.stride,
    ...
    def __repr__(self) -> str:
        ...
        repr_string = f'LoRAConvTranspose2d({self.base_module} + ((α={self.alpha.item()}/r={self.rank.item()}) × {adapter_repr_string}))'
        if self.delta_bias is not None:
            repr_string += f' + ({self.beta.item()} × delta_bias)'
        return repr_string
